...
let captureOccured = {
    ...
    tile: undefined,
    tileStep: undefined,
    execute: function(row = this.row, rowStep = this.rowStep, tile = this.tile, tileStep = this.tileStep, captureType) {
        switch (captureType) {
            case 1: {
                break;
            }
...
function checkMovement(row, tile) {
    if (tiles[row][tile].classList.contains("suggested-move")) { // if a suggested path is chosen
        if (captureOccured) {
            captureOccured();
        }
        else {
...
function clearSuggestions() {
    for (let i = 1; i < tiles.length; i++) {
        for(let j = 0; j < 4; j++) {
            tiles[i][j].classList.remove("suggested-move");
        }
    }
}

function oldShowSuggestions(row, tile, rowStep, tileStep) {
    if(tiles[row+rowStep][tile+tileStep] && !tiles[row+rowStep][tile+tileStep].classList.contains("black-pawn") && !tiles[row+rowStep][tile+tileStep].classList.contains("white-pawn")) { // makes sure that suggested area isn't taken
        tiles[row+rowStep][tile+tileStep].classList.add("suggested-move");
    }
    if(tiles[row+rowStep][tile] && !tiles[row+rowStep][tile].classList.contains("black-pawn") && !tiles[row+rowStep][tile].classList.contains("white-pawn")) { // makes sure that suggested area isn't taken
        tiles[row+rowStep][tile].classList.add("suggested-move");
    }
}

function showSuggestions(row, tile, rowStep, tileStep) { // currently designed only for black turn
    if (turn == "black-pawn") { // black turn
        if (tiles[row+rowStep][tile+tileStep] && !tiles[row+rowStep][tile+tileStep].classList.contains("black-pawn")) {
            if (tiles[row+rowStep][tile+tileStep].classList.contains("white-pawn")) {
                if (tiles[row+rowStep*2][tile+tileStep] && !tiles[row+rowStep*2][tile+tileStep].classList.contains("black-pawn") && !tiles[row+rowStep*2][tile+tileStep].classList.contains("white-pawn")) {
                    captureOccured = function(row, rowStep, tile, tileStep) {
                    tiles[row+rowStep*2][tile+tileStep].classList.add("suggested-move");
                    tiles[row+rowStep][tile+tileStep].classList.remove("white-pawn");
                    whitePawns--;
                }  
            }
            else {
        ...
        if (tiles[row+rowStep][tile] && !tiles[row+rowStep][tile].classList.contains("black-pawn")) {
            if (tiles[row+rowStep][tile].classList.contains("white-pawn")) {
                if (tiles[row+rowStep*2][tile+tileStep*(-1)] && !tiles[row+rowStep*2][tile+tileStep*(-1)].classList.contains("black-pawn") && !tiles[row+rowStep*2][tile+tileStep*(-1)].classList.contains("white-pawn")) {
                    tiles[row+rowStep*2][tile+tileStep*(-1)].classList.add("suggested-move");
                    tiles[row+rowStep][tile].classList.remove("white-pawn");
                    whitePawns--;
                } 
            }
            else {
        ...
        if (tiles[row+rowStep][tile+tileStep] && !tiles[row+rowStep][tile+tileStep].classList.contains("white-pawn")) {
            if (tiles[row+rowStep][tile+tileStep].classList.contains("black-pawn")) {
                if (tiles[row+rowStep*2][tile+tileStep] && !tiles[row+rowStep*2][tile+tileStep].classList.contains("black-pawn") && !tiles[row+rowStep*2][tile+tileStep].classList.contains("white-pawn")) {
                    tiles[row+rowStep*2][tile+tileStep].classList.add("suggested-move");
                    tiles[row+rowStep][tile+tileStep].classList.remove("black-pawn");
                    blackPawns--;
                }  
            }
            else {
        ...
        if (tiles[row+rowStep][tile] && !tiles[row+rowStep][tile].classList.contains("white-pawn")) {
            if (tiles[row+rowStep][tile].classList.contains("black-pawn")) {
                if (tiles[row+rowStep*2][tile+tileStep*(-1)] && !tiles[row+rowStep*2][tile+tileStep*(-1)].classList.contains("black-pawn") && !tiles[row+rowStep*2][tile+tileStep*(-1)].classList.contains("white-pawn")) {
                    tiles[row+rowStep*2][tile+tileStep*(-1)].classList.add("suggested-move");
                    tiles[row+rowStep][tile].classList.remove("black-pawn");
                    blackPawns--;
                } 
            }
            else {
